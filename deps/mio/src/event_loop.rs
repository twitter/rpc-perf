...
impl<H: Handler> EventLoop<H> {
    ...
    pub fn configured(config: EventLoopConfig) -> io::Result<EventLoop<H>> {
        ...
        // Create the timer
        let mut timer = Timer::new(config.timer_tick_ms,
    ...
    /// Registers an IO handle with the event loop.
    pub fn register_opt<E: ?Sized>(&mut self,
                                   io: &E,
                                   token: Token,
                                   interest: EventSet,
                                   opt: PollOpt)
    ...
    pub fn reregister<E: ?Sized>(&mut self,
                                 io: &E,
                                 token: Token,
                                 interest: EventSet,
                                 opt: PollOpt)
...
mod tests {
    ...
    impl Handler for Funtimes {
        ...
        fn ready(&mut self,
                 _event_loop: &mut EventLoop<Funtimes>,
                 token: Token,
                 events: EventSet) {
