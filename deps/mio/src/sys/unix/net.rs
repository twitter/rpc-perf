...
pub fn shutdown(io: &Io, how: Shutdown) -> io::Result<()> {
    ...
    nix::shutdown(io.as_raw_fd(), how).map_err(super::from_nix_error)
}
...
pub fn set_nodelay(io: &Io, delay: bool) -> io::Result<()> {
    nix::setsockopt(io.as_raw_fd(), nix::sockopt::TcpNoDelay, &delay).map_err(super::from_nix_error)
}
...
pub fn getpeername(io: &Io) -> io::Result<nix::SockAddr> {
    nix::getpeername(io.as_raw_fd()).map_err(super::from_nix_error)
}
