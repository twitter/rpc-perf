...
//! # Goals
//!
//! * Fast - minimal overhead over the equivalent OS facilities (epoll, kqueue,
//! etc...)
//! * Zero allocations
//! * A scalable readiness-based API, similar to epoll on Linux
//! * Design to allow for stack allocated buffers when possible (avoid double
//! buffering).
//! * Provide utilities such as a timers, a notification channel, buffer
//! abstractions, and a slab.
//!
//! # Usage
//!
...
//!     type Message = ();
//!
//! fn ready(&mut self, event_loop: &mut EventLoop<MyHandler>, token:
//! Token, _: EventSet) {
//!         match token {
//!             SERVER => {
...
pub use event::{PollOpt, EventSet};
pub use event_loop::{EventLoop, EventLoopConfig, Sender};
pub use handler::Handler;
pub use io::{TryRead, TryWrite, Evented, TryAccept};
...
// Re-export bytes
pub mod buf {
    pub use bytes::{Buf, MutBuf, ByteBuf, MutByteBuf, RingBuf, SliceBuf, MutSliceBuf};
}
